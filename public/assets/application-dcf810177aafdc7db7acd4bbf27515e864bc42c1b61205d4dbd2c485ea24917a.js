...
(function() {
  $(document).ready(function() {
    ...
    return $("#publish_btn").on("click", function(e) {
      var f;
      f = $(this).closest("form");
...
(function() {
  $(document).ready(function() {
    ...
    return $("form#new_enrollment").submit(function(e) {
      var button, res;
      e.preventDefault();
      ...
      res.done(function() {
        ...
        return document.location.href = SUCCESS_URL;
      });
      return res.fail(function() {
        var errors;
        button.removeAttr("disabled");
        errors = JSON.parse(res.responseText)['errors'];
        return $.each(errors, function(k, v, i) {
          $("#enrollment_" + k).css("border", "1px solid red");
          return $("#enrollment_" + k + "_error").html("That " + k + " " + v[0]);
        });
      });
    });
  });
  ...
  $(document).ready(function() {
    ...
    if (hms.length > 1) {
      ...
      if (pafm.length > 1) {
        ...
        return $.post("affiliate_landings", {
          affiliate_id: afid
        });
      }
    }
...
(function() {
  $(document).ready(function() {
    return $("#blog .article-html table").addClass("table-responsive");
  });
...
(function() {
  ...
  homeData = [
    {
      ...
      "p": "At devschool we call it <strong>Enrollment to Hire</strong> education and <strong>we mean it</strong>.",
      "as": "NPR",
      ...
      "h2": "At devschool you will NOT be taught by an ex student",
  ...
  $(document).ready(function() {
    var bgs, menuItems, scrollItems, topMenu, topMenuHeight;
    bgs = [1, 2, 3, 5, 6].sort(function() {
      return .5 - Math.random();
    });
    $(".random-person").each(function(i, el) {
      return $(el).addClass("person-" + bgs[i]);
    });
    smoothScroll.init({
      speed: 1000,
    ...
    $(document).find("[data-featherlight]").featherlight();
    topMenu = $("#top-menu");
    ...
    return $(window).scroll(function() {
      ...
      return menuItems.parent().removeClass("active").end().filter("[href=#" + id + "]").parent().addClass("active");
    });
  });
...
(function() {
  $(document).ready(function() {
    var handleStripeResponse;
    handleStripeResponse = function(form, button, status, response) {
      var input, res, token, url;
      if (response.error) {
        button.removeAttr("disabled");
        return $(".payment-errors").html(response.error.message);
      } else {
        token = response['id'];
        ...
        res.done(function() {
          var html;
          ga('create', 'UA-66453024-3', 'auto');
          ga('send', {
            hitType: 'event',
            eventCategory: 'Payments',
            eventAction: 'payment-made',
            eventLabel: 'Enrollment payment'
          });
          html = ["<div class='page-header text-center'>", "<h2>Welcome to Devschool!</h2>", "<h4>Your payment was successfully processed</h4>", "</div>", "<p>Jim OKelly will be your new web development instructor!", "He will email you today with instructions on how to proceed and enter the chat room.</p>"];
          console.debug(html);
          return $(".pane").html(html.join("\n"));
        });
        return res.fail(function() {
          $(".payment-errors").html("There was an error charging your card. Please initiate a live chat on this page to fix the issue.");
          return console.debug(JSON.parse(res.responseText));
        });
      }
    };
    $("input.card-number").payment('formatCardNumber');
    return $("form#new_payment").on("submit", function(e) {
      var attrs, button, form;
      form = $(this);
      ...
      return Stripe.createToken(attrs, function(status, response) {
        return handleStripeResponse(form, button, status, response);
      });
    });
  });
